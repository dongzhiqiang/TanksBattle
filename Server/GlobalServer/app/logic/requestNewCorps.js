...
var doLogicCoroutine = Promise.coroutine(function * (req, res, body, pathName) {
    ...
    try {
        ...
        var ret = yield col2.findOneAndUpdate({_id:0}, {$inc:{corpsId:1}}, {projection:{corpsId:1}, upsert:true, returnOriginal:false});
        //得到唯一公会实例ID
        var corpsId = ret.corpsId;
        col2 = null;

        //收集参数
        var updateObj = {};
        updateObj.corpsId = corpsId;
        ...
        retMsg.code = 0;
        retMsg.msg = "OK";
        retMsg.cxt = {corpsId:corpsId}; //返回实例ID
    }
    catch (err) {
        retMsg.code = 1;
        retMsg.msg = "服务器异常";
