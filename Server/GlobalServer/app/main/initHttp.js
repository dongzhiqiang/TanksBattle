...
function doInit() {
    return new Promise(function (resolve, reject) {
        ...
        var httpSvr = http.createServer(procFun);
        httpSvr.listen(appCfg.port, appCfg.bindIP, function(){
            logUtil.info("HTTP监听于端口" + appCfg.port);
        ...
        //创建HTTPS服务
        logUtil.info("HTTPS服务开始初始化...");
        var options = {
            key: fs.readFileSync("data/http_certs/key.pem"),
            cert: fs.readFileSync("data/http_certs/cert.pem")
        };
        var httpsSvr = https.createServer(options, procFun);
        httpsSvr.listen(appCfg.port4Https, appCfg.bindIP, function(){
            logUtil.info("HTTPS监听于端口" + appCfg.port4Https);
            logUtil.info("HTTPS服务完成初始化");
            if (++launchOKCnt >= 2)
                resolve();
        });
        httpsSvr.on("error", function(err){
            reject(err);
        });
    });
}
...
function doDestroy()
{
    return new Promise(function (resolve, reject) {
        ...
        logUtil.info("HTTPS服务开始销毁...");
        if (!httpsSvr) {
            logUtil.info("HTTPS服务完成销毁");
            if (++closeCnt >= 2)
                resolve();
        }
        else {
            //防止再有错误
            httpsSvr.on("error", function(err){
                logUtil.warn("HTTPS服务出错", err);
            });
            httpsSvr.close(function () {
                httpsSvr = null;
                logUtil.info("HTTPS服务完成销毁");
                if (++closeCnt >= 2)
                    resolve();
