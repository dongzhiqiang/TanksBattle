...
public class GetOrCreateTest {
	...
	public void test() throws InterruptedException {
		...
		for (int i = 0; i < times; i++) {
			...
			Thread thread1 = new Thread() {
				public void run() {
					...
					try {
						itemService.getOrCreate(IndexValue.valueOf("owner", 1), id,
								new EntityBuilder<Integer, BasicItem>() {
									public BasicItem newInstance(Integer id) {
										return BasicItem.valueOf(id, 1, 0);
									}
								});
					} catch (Exception e) {
			...
			Thread thread2 = new Thread() {
				public void run() {
					...
					try {
						itemService.getOrCreate(IndexValue.valueOf("owner", 2), id,
								new EntityBuilder<Integer, BasicItem>() {
									public BasicItem newInstance(Integer id) {
										return BasicItem.valueOf(id, 2, 0);
									}
								});
					} catch (Exception e) {
						errors.incrementAndGet();
		...
		Assert.assertThat(errors.get(), CoreMatchers.is(0));
