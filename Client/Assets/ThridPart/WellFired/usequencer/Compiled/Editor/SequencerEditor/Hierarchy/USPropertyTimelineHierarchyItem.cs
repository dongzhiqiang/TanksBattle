...
namespace WellFired
{
	...
	public class USPropertyTimelineHierarchyItem : IUSTimelineHierarchyItem 
	{
		...
		public float GetPreviousShownKeyframeTime(float runningTime)
		{
            ...
            foreach (var curve in CurveEditor.Curves)
            {
                float curvePrveKeyFrameTime = curve.FindPrevKeyframeTime(runningTime);
                if (previousPropertyTime < curvePrveKeyFrameTime)
                    previousPropertyTime = curvePrveKeyFrameTime;
            }
		...
		public float GetNextShownKeyframeTime(float runningTime)
		{
            ...
            foreach( var curve in CurveEditor.Curves)
            {
                float curveNextKeyFrameTime = curve.FindNextKeyframeTime(runningTime);
                if (curveNextKeyFrameTime < nextPropertyTime)
                    nextPropertyTime = curveNextKeyFrameTime;
            }
