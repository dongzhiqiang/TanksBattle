...
#endif
	{
...
#endif
		{
			...
			case GradientFillMode.PerQuad:
				...
				{
					ApplyTint(uiVertices, i, m_TopColor);
					ApplyTint(uiVertices, i + 1, m_TopColor);
					ApplyTint(uiVertices, i + 2, m_BottomColor);
					ApplyTint(uiVertices, i + 3, m_BottomColor);
					ApplyTint(uiVertices, i + 4, m_BottomColor);
					ApplyTint(uiVertices, i + 5, m_TopColor);
				}
	#if IS_VBO_MESH
			...
			case GradientFillMode.GeometryBounds:
				yMin = uiVertices[0].position.y;
				yMax = uiVertices[0].position.y;
				for (int i = 1; i < count; i++)
				{
					float y = uiVertices[i].position.y;
					if (y > yMax)
					{
						yMax = y;
					}
			...
			{
				ApplyTint(
					vertices, i, Color32.Lerp(m_TopColor, m_BottomColor, (yMax - uiVertices[i].position.y) * div)
				);
			}
#else
			for (int i = 0; i < count; ++i)
			{
				ApplyTint(
					uiVertices, i, Color32.Lerp(m_TopColor, m_BottomColor, (yMax - uiVertices[i].position.y) * div)
				);
			}
	#if IS_VBO_MESH
			using (UnityEngine.UI.VertexHelper vh = new UnityEngine.UI.VertexHelper())
