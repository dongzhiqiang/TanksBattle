	...
	public class KeywordsGlossary : KeywordCollection
	{
		...
		protected override void PopulateKeywordList(List<string> keywordList)
		{
			for (int i = 0; i < m_Entries.Count; ++i)
			{
				...
				List<InflectedForm> otherForms = ListPool<InflectedForm>.Get();
				m_Entries[i].GetOtherForms(ref otherForms);
				for (int j = 0; j < otherForms.Count; ++j)
				{
					keywordList.Add(otherForms[j].Word);
				}
				ListPool<InflectedForm>.Release(otherForms);
			}
			RebuildEntryTable(ref m_EntryTable);
		}
		...
		private void RebuildEntryTable(ref Dictionary<string, int> entryTable)
		{
			...
			for (int i = m_Entries.Count - 1; i >= 0; --i)
			{
				...
				List<InflectedForm> otherForms = ListPool<InflectedForm>.Get();
				m_Entries[i].GetOtherForms(ref otherForms);
				for (int j = otherForms.Count - 1; j >= 0; --j)
				{
					keyword = otherForms[j].Word;
					if (ignoreCase)
					{
						keyword = keyword.ToLower();
					}
