...
namespace Candlelight.UI
{
	...
	public class HyperTextProcessor : System.IDisposable, ITextSource
	{
		...
		/// </summary>
		public event ITextSourceEvent OnBecameDirty;
			...
			set
			{
				if (
					BackingFieldUtility.SetInterfaceBackingField(
						...
						onAssign: t => t.OnBecameDirty += SetDirty,
						onUnassign: t => t.OnBecameDirty -= SetDirty
					)
				)
				{
					SetDirty();
		...
		public void OnEnable()
		{
			...
			{
				m_InputTextSource.OnBecameDirty += SetDirty;
				m_InputTextSource.OnBecameDirty -= SetDirty;
			}
			SetDirty();
		}
		...
		private void ProcessInputText()
		{
			...
			StringBuilder processedTextBuilder = StringX.GetStringBuilder();
			int indexInRawString = 0;
			Dictionary<string, HyperTextStyles.Text> customTags = DictPool<string, HyperTextStyles.Text>.Get();
			if (m_Styles != null)
			{
				List<HyperTextStyles.Text> cascadedTextStyles = ListPool<HyperTextStyles.Text>.Get();
				m_Styles.GetCascadedCustomTextStyles(ref cascadedTextStyles);
				for (int i = 0; i < cascadedTextStyles.Count; ++i)
				{
					if (
						!string.IsNullOrEmpty(cascadedTextStyles[i].Tag) &&
						!customTags.ContainsKey(cascadedTextStyles[i].Tag)
					)
					{
						customTags.Add(cascadedTextStyles[i].Tag, cascadedTextStyles[i]);
					}
				}
				ListPool<HyperTextStyles.Text>.Release(cascadedTextStyles);
			}
			// insert tags in text for words present in keyword collections
			string textCache = SubstituteTagsInForKeywords(this.InputTextToUse);
			// if rich text is enabled, substitute quad arguments, discrete sizes, and custom tag styles into text
			Dictionary<string, HyperTextStyles.Link> linkStyles = DictPool<string, HyperTextStyles.Link>.Get();
			List<HyperTextStyles.Quad> cascadedQuadStyles = ListPool<HyperTextStyles.Quad>.Get();
			if (m_Styles != null)
			{
				m_Styles.GetCascadedQuadStyles(ref cascadedQuadStyles);
			}
			m_CustomTags.Clear();
			m_Quads.Clear();
			Dictionary<IndexRange, float> processedIndexRangesAndScalars = DictPool<IndexRange, float>.Get();
			Dictionary<IndexRange, int> indexRangeOffsets = DictPool<IndexRange, int>.Get();
			if (this.IsRichTextEnabled)
			{
				// sub quad arguments into text
				textCache =
					s_QuadTagRegex.Replace(textCache, match => GetPostprocessedQuadTag(match, cascadedQuadStyles));
						...
						textCache = s_TagRegexes[tag].Replace(
							...
							delegate(Match match)
							{
								...
								IndexRange characterIndices = new IndexRange(
									match.Index + openTag.Length, match.Index + openTag.Length + segment.Length - 1
								);
								indexRangeOffsets.Clear();
								indexRangeOffsets.Add(
									...
									openTag.Length - match.Groups[OpenTagCaptureGroup].Length
								);
								indexRangeOffsets.Add(
								...
								{
									range.Offset(indexRangeOffsets);
								}
				...
				// collect link styles
				List<HyperTextStyles.LinkSubclass> cascadedLinkStyles = ListPool<HyperTextStyles.LinkSubclass>.Get();
				this.Styles.GetCascadedLinkStyles(ref cascadedLinkStyles);
				for (int i = 0; i < cascadedLinkStyles.Count; ++i)
				{
					if (
						!string.IsNullOrEmpty(cascadedLinkStyles[i].ClassName) &&
						!linkStyles.ContainsKey(cascadedLinkStyles[i].ClassName)
					)
					{
						linkStyles.Add(cascadedLinkStyles[i].ClassName, cascadedLinkStyles[i].Style);
					}
				}
				ListPool<HyperTextStyles.LinkSubclass>.Release(cascadedLinkStyles);
			}
			DictPool<string, HyperTextStyles.Text>.Release(customTags);
			...
			foreach (Match match in s_PostprocessedLinkTagRegex.Matches(textCache))
			{
				...
				processedTextBuilder.Append(textCache.Substring(indexInRawString, match.Index - indexInRawString));
				...
				// append substitution
				processedTextBuilder.Append(result);
				indexInRawString = match.Index + match.Length;
				// store the data for the link
				int startPosition = processedTextBuilder.Length -
			...
			DictPool<string, HyperTextStyles.Link>.Release(linkStyles);
			processedTextBuilder.Append(textCache.Substring(indexInRawString, textCache.Length - indexInRawString));
			m_OutputText = processedTextBuilder.ToString();
			...
			if (this.IsRichTextEnabled)
			{
				...
				if (this.IsDynamicFontEnabled)
				{
					...
					foreach (KeyValuePair<IndexRange, float> rangeScalar in processedIndexRangesAndScalars)
					{
						...
						string segment = m_OutputText.Substring(rangeScalar.Key.StartIndex, rangeScalar.Key.Count);
						int oldLength = segment.Length;
						if (s_PostProcessedSizeAttributeRegex.IsMatch(segment))
						{
							processedTextBuilder = new StringBuilder();
							processedTextBuilder.Append(m_OutputText.Substring(0, rangeScalar.Key.StartIndex));
							...
							processedTextBuilder.Append(segment);
							processedTextBuilder.Append(m_OutputText.Substring(rangeScalar.Key.EndIndex + 1));
							m_OutputText = processedTextBuilder.ToString();
							int delta = segment.Length - oldLength;
							if (delta != 0)
							{
								indexRangeOffsets.Clear();
								indexRangeOffsets.Add(rangeScalar.Key, delta);
								foreach (IndexRange range in processedIndexRangesAndScalars.Keys)
								{
				...
				foreach (Match match in s_QuadTagRegex.Matches(m_OutputText))
				{
					...
					int templateIndex = cascadedQuadStyles.FindIndex(quad => quad.ClassName == quadName);
					...
					{
						m_Quads.Add(
							new Quad(new IndexRange(quadGeomIndex, quadGeomIndex), cascadedQuadStyles[templateIndex])
						);
					}
				}
			}
			ListPool<HyperTextStyles.Quad>.Release(cascadedQuadStyles);
			DictPool<IndexRange, float>.Release(processedIndexRangesAndScalars);
			DictPool<IndexRange, int>.Release(indexRangeOffsets);
			StringX.ReleaseStringBuilder(processedTextBuilder);
		...
		{
			m_IsDirty = true;
			if (this.OnBecameDirty != null)
			{
				this.OnBecameDirty.Invoke(this);
			}
		}
		...
		private void SetDirty(ITextSource textSource)
		{
			SetDirty();
		}
		...
		private string SubstituteTagsInForKeywords(string input)
		{
			...
			HashSet<string> processedKeywords = HashPool<string>.Get();
			...
			foreach (KeyValuePair<KeywordCollection, System.Func<string, string, string>> kv in keywordCollections)
			{
				...
				{
					if (processedKeywords.Contains(keyword) || string.IsNullOrEmpty(keyword))
					{
						continue;
					}
					...
					{
						processedTextBuilder.Remove(0, processedTextBuilder.Length);
						for (int matchIndex = 0; matchIndex < tagMatches.Count; ++matchIndex)
						{
							...
							// append patched text
							processedTextBuilder.Append(kv.Value(segment, keyword));
							// append tag
							processedTextBuilder.Append(tagMatches[matchIndex].Value);
							// append segment following final tag
							if (matchIndex == tagMatches.Count - 1)
							{
								segment = input.Substring(tagMatches[matchIndex].Index + tagMatches[matchIndex].Length);
								processedTextBuilder.Append(kv.Value(segment, keyword));
							}
						}
						input = processedTextBuilder.ToString();
					}
			...
			StringX.ReleaseStringBuilder(processedTextBuilder);
			HashPool<string>.Release(processedKeywords);
			return input;
		}
	}
}