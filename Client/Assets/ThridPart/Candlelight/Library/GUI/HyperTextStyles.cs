...
#pragma warning restore 219
		...
		private void RebuildCascadingAndInheritedStyles()
		{
			...
			// rebuild cascades
			Dictionary<LinkSubclass, HyperTextStyles> cascadedLinkStyles =
				DictPool<LinkSubclass, HyperTextStyles>.Get();
			Dictionary<Quad, HyperTextStyles> cascadedQuadStyles = DictPool<Quad, HyperTextStyles>.Get();
			Dictionary<Text, HyperTextStyles> cascadedTextStyles = DictPool<Text, HyperTextStyles>.Get(); 
			AddToCascade(m_InheritedLinkStyles, cascadedLinkStyles);
			AddToCascade(m_InheritedQuadStyles, cascadedQuadStyles);
			AddToCascade(m_InheritedTextStyles, cascadedTextStyles);
			// ensure duplicate entries (via inspector) don't get double-added to dictionaries for this instance
			Dictionary<LinkSubclass, HyperTextStyles> selfLinkStyles = DictPool<LinkSubclass, HyperTextStyles>.Get();
			Dictionary<Quad, HyperTextStyles> selfQuadStyles = DictPool<Quad, HyperTextStyles>.Get();
			Dictionary<Text, HyperTextStyles> selfTextStyles = DictPool<Text, HyperTextStyles>.Get(); 
			foreach (LinkSubclass style in m_LinkStyles)
			{
				if (selfLinkStyles.ContainsKey(style))
				{
					continue;
				}
				selfLinkStyles.Add(style, this);
			}
			foreach (Quad style in m_QuadStyles)
			{
				if (selfQuadStyles.ContainsKey(style))
				{
					continue;
				}
				selfQuadStyles.Add(style, this);
			}
			foreach (Text style in m_CustomTextStyles)
			{
				if (selfTextStyles.ContainsKey(style))
				{
					continue;
				}
				selfTextStyles.Add(style, this);
			}
			AddToCascade(selfLinkStyles, cascadedLinkStyles);
			AddToCascade(selfQuadStyles, cascadedQuadStyles);
			AddToCascade(selfTextStyles, cascadedTextStyles);
			m_CascadedLinkStyles.Clear();
			m_CascadedQuadStyles.Clear();
			m_CascadedTextStyles.Clear();
			m_CascadedLinkStyles.AddRange(cascadedLinkStyles.Keys);
			m_CascadedQuadStyles.AddRange(cascadedQuadStyles.Keys);
			m_CascadedTextStyles.AddRange(cascadedTextStyles.Keys);
			// clean up
			DictPool<LinkSubclass, HyperTextStyles>.Release(selfLinkStyles);
			DictPool<Quad, HyperTextStyles>.Release(selfQuadStyles);
			DictPool<Text, HyperTextStyles>.Release(selfTextStyles);
			DictPool<LinkSubclass, HyperTextStyles>.Release(cascadedLinkStyles);
			DictPool<Quad, HyperTextStyles>.Release(cascadedQuadStyles);
			DictPool<Text, HyperTextStyles>.Release(cascadedTextStyles);
