...
namespace Candlelight
{
	...
	public class FlushChildrenDrawer : PropertyDrawer
	{
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			{
				SerializedProperty spOnGUI = property.Copy();
				spOnGUI.NextVisible(true);
				Regex match = new Regex(string.Format("^{0}(?=\\.)", Regex.Escape(property.propertyPath)));
				while (match.IsMatch(spOnGUI.propertyPath))
				{
					s_Label.text = spOnGUI.displayName;
					position.height = EditorGUI.GetPropertyHeight(spOnGUI, s_Label, true);
					EditorGUI.PropertyField(position, spOnGUI, true);
					position.y += position.height + EditorGUIUtility.standardVerticalSpacing;
					spOnGUI.NextVisible(false);
				}
			}
		}
	}
