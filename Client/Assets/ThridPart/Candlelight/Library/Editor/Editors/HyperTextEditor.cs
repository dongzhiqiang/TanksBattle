...
namespace Candlelight.UI
{
	...
	public static class HyperTextUtility
	{
		...
		/// </summary>
		[MenuItem("Assets/Candlelight/Clean Up HyperText Quads")]
...
#pragma warning disable 612
			...
			List<string> assetPaths =
				(from guid in AssetDatabase.FindAssets("t:Prefab") select AssetDatabase.GUIDToAssetPath(guid)).ToList();
			for (int i = 0; i < assetPaths.Count; ++i)
			{
				string assetPath = assetPaths[i];
				string message = string.Format("Processing Prefab {0} / {1}", i, assetPaths.Count);
				EditorUtility.DisplayProgressBar(message, message, (float)i / assetPaths.Count);
				...
				Undo.SetCurrentGroupName("Clean Up HyperText Quads in Prefabs");
#endif
				foreach (HyperTextQuad quad in prefab.GetComponentsInChildren<HyperTextQuad>(true))
				{
					Undo.DestroyObjectImmediate(quad);
				}
			}
			...
			if (
				!EditorUtility.DisplayDialog(
					"Clean Up Scenes?",
					"Prefab hierarchies have been cleaned.\n\n" +
					"Do you also wish to clean up all HyperTextQuads in scenes that are not in prefab hierarchies? " +
					"(This operation is not undoable and will clear your undo queue.)",
					"Yes", "No"
				)
			)
			{
			...
			assetPaths.RemoveAll(p => !System.IO.File.Exists(p));
			for (int i = 0; i < assetPaths.Count; ++i)
			{
				string scenePath = assetPaths[i];
				string message = string.Format("Processing Scene {0} / {1}", i, assetPaths.Count);
				EditorUtility.DisplayProgressBar(message, message, (float)i / assetPaths.Count);
#if NO_SCENE_MANAGER
...
#endif
				HashSet<HyperTextQuad> allQuads = new HashSet<HyperTextQuad>();
				foreach (GameObject go in Object.FindObjectsOfType<GameObject>())
				{
					foreach (HyperTextQuad quad in go.GetComponentsInChildren<HyperTextQuad>(true))
					{
						allQuads.Add(quad);
					}
				}
				foreach (HyperTextQuad quad in allQuads)
				{
					Undo.DestroyObjectImmediate(quad);
				}
				if (allQuads.Count > 0)
...
#pragma warning restore 612
	...
	public class HyperTextEditor : TextExEditor
    {
		/// <summary>
        ...
        private SerializedProperty m_MinPreferredWidth;
        private SerializedProperty m_MaxPreferredWidth;
        #endregion
        /// <summary>
		...
		new private void OnEnable()
		{
			...
			m_AlignByGeometry = m_FontDataProperty.FindPropertyRelative("m_AlignByGeometry");
            ...
            m_MinPreferredWidth = this.serializedObject.FindProperty("m_minPreferredWidth");
            m_MaxPreferredWidth = this.serializedObject.FindProperty("m_maxPreferredWidth");
		...
		public override void OnInspectorGUI()
		{
            ...
            EditorGUILayout.PropertyField(m_MinPreferredWidth);
            EditorGUILayout.PropertyField(m_MaxPreferredWidth);
