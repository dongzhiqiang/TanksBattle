    ...
    public class XffectComponent : MonoBehaviour
    {
        ...
        public void FindMyCamera()
        {
            ...
            if (Main.instance != null)
            {
                ...
                {
                    Debuger.LogError("震动找不到相机");
                    return;
                }

                if (CameraMgr.instance.CurCamera == null)
                {
                    Debuger.LogError("震动找不到相机，相机管理器的当前相机为空");
                    return;
                }
                mCamera = CameraMgr.instance.CurCamera;
                return;
            }
#endif
            ...
            for (int i = 0, imax = cameras.Length; i < imax; ++i)
            {
                ...
                if ((cam.cullingMask & layerMask) != 0 && !cam.name.Contains("ui") && !cam.name.Contains("UI"))//这里要排除ui相机
                {
                    mCamera = cam;
                    return;
                }
        ...
        void OnEnable()
        {
            ...
            if (!GameObjectPool.IsPreloading())
            {
                Active();
            }
        }
