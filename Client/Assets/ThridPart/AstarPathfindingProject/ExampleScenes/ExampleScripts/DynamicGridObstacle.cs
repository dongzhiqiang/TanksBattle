...
public class DynamicGridObstacle : MonoBehaviour {
	...
	IEnumerator UpdateGraphs () {
		...
		if (col == null ) {
			Debug.LogError ("No collider is attached to the GameObject. Canceling check");
			yield break;
		}
		...
		while (col) {
            ...
            if (AstarPath.active == null /*|| !RolePath.s_canStuckAvoid*/)
            {
                yield return new WaitForSeconds(checkTime);
                continue;
            }
