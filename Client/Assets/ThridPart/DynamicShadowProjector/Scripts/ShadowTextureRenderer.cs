...
namespace DynamicShadowProjector {
	...
	public class ShadowTextureRenderer : MonoBehaviour {
		...
		[SerializeField]
		private bool m_testViewClip = false;
		[SerializeField]
		...
		bool Initialize()
		{
			...
			if (m_camera == null) {
                ...
                m_camera.hideFlags = HideFlags.HideInInspector | HideFlags.HideAndDontSave;
			}
			else {
		...
		void Awake()
		{
            m_testViewClip = false;//强制取消viewclip，不然就必须设置一个相机进来
			Initialize();
		}
		...
		void Start()
		{
...
#endif
			{
                ...
                //    if (caMgr != null && caMgr.CurCamera!= null)
                //        m_camerasForViewClipTest = new Camera[1] { caMgr.CurCamera };
                //}
			}
		}
		...
		void CloneProjectorMaterialIfShared()
		{
            ...
            //if (this.transform.parent.parent.name.StartsWith("mod_kratos_02"))
            //{
            //    Debuger.LogError("创建奎爷阴影");
            //}
            //if (s_sharedMaterials.Contains(m_projector.material))//FIX,如果这个游戏对象是Object.Instantiate而来，那么到这里的判断就会有问题,改成下面这行
#if !ART_DEBUG
		...
		void OnDestroy()
		{
			if (m_projectorMaterial != null) {
				...
				if (m_projectorMaterial.hideFlags == CLONED_MATERIAL_HIDE_FLAGS) {
                    ...
                    //if (this.transform.parent.parent.name.StartsWith("mod_kratos_02"))
                    //{
                    //    Debuger.LogError("删除奎爷阴影");
                    //}
#if !ART_DEBUG
                    if (Main.instance == null)
#endif
                    {
                        DestroyImmediate(m_projectorMaterial);//FIX,如果这个游戏对象是Object.Instantiate而来，那么到这里的判断就会有问题,改成下面这行，那么删除之前的对象的时候可能材质还没有克隆，这个时候材质就会丢失
                    }
...
#if UNITY_EDITOR
			else if (m_testViewClip && m_camerasForViewClipTest != null && 0 < m_camerasForViewClipTest.Length && Application.isPlaying)
#else
...
//            //主相机的裁剪,这里不做这个优化了，因为过场动画播放的时候取不到过场动画的相机
//            Camera caMain = CameraMgr.instance!=null?CameraMgr.instance.CurCamera:null;
//#if UNITY_EDITOR
//            if (m_testViewClip && caMain!=null && Application.isPlaying)
//#else
//            if (m_testViewClip && caMain!=null)
//#endif
//            {
//                Vector3 v0 = m_camera.ViewportToWorldPoint(new Vector3(0, 0, m_camera.nearClipPlane));
//                Vector3 v1 = m_camera.ViewportToWorldPoint(new Vector3(1, 0, m_camera.nearClipPlane));
...
//                Vector3 min = caMain.WorldToViewportPoint(v0);
//                if (min.z < 0.0f) { min.x = -min.x; min.y = -min.y; }
//                Vector3 max = min;
//                Vector3 v = caMain.WorldToViewportPoint(v1);
//                if (v.z < 0.0f) { v.x = -v.x; v.y = -v.y; }
//                min.x = Mathf.Min(min.x, v.x); min.y = Mathf.Min(min.y, v.y); min.z = Mathf.Min(min.z, v.z);
//                max.x = Mathf.Max(max.x, v.x); max.y = Mathf.Max(max.y, v.y); max.z = Mathf.Max(max.z, v.z);
//                v = caMain.WorldToViewportPoint(v2);
//                if (v.z < 0.0f) { v.x = -v.x; v.y = -v.y; }
//                min.x = Mathf.Min(min.x, v.x); min.y = Mathf.Min(min.y, v.y); min.z = Mathf.Min(min.z, v.z);
//                max.x = Mathf.Max(max.x, v.x); max.y = Mathf.Max(max.y, v.y); max.z = Mathf.Max(max.z, v.z);
//                v = caMain.WorldToViewportPoint(v3);
//                if (v.z < 0.0f) { v.x = -v.x; v.y = -v.y; }
//                min.x = Mathf.Min(min.x, v.x); min.y = Mathf.Min(min.y, v.y); min.z = Mathf.Min(min.z, v.z);
//                max.x = Mathf.Max(max.x, v.x); max.y = Mathf.Max(max.y, v.y); max.z = Mathf.Max(max.z, v.z);
//                v = caMain.WorldToViewportPoint(v4);
//                if (v.z < 0.0f) { v.x = -v.x; v.y = -v.y; }
//                min.x = Mathf.Min(min.x, v.x); min.y = Mathf.Min(min.y, v.y); min.z = Mathf.Min(min.z, v.z);
//                max.x = Mathf.Max(max.x, v.x); max.y = Mathf.Max(max.y, v.y); max.z = Mathf.Max(max.z, v.z);
//                v = caMain.WorldToViewportPoint(v5);
//                if (v.z < 0.0f) { v.x = -v.x; v.y = -v.y; }
//                min.x = Mathf.Min(min.x, v.x); min.y = Mathf.Min(min.y, v.y); min.z = Mathf.Min(min.z, v.z);
//                max.x = Mathf.Max(max.x, v.x); max.y = Mathf.Max(max.y, v.y); max.z = Mathf.Max(max.z, v.z);
//                v = caMain.WorldToViewportPoint(v6);
//                if (v.z < 0.0f) { v.x = -v.x; v.y = -v.y; }
//                min.x = Mathf.Min(min.x, v.x); min.y = Mathf.Min(min.y, v.y); min.z = Mathf.Min(min.z, v.z);
//                max.x = Mathf.Max(max.x, v.x); max.y = Mathf.Max(max.y, v.y); max.z = Mathf.Max(max.z, v.z);
//                v = caMain.WorldToViewportPoint(v7);
//                if (v.z < 0.0f) { v.x = -v.x; v.y = -v.y; }
//                min.x = Mathf.Min(min.x, v.x); min.y = Mathf.Min(min.y, v.y); min.z = Mathf.Min(min.z, v.z);
//                max.x = Mathf.Max(max.x, v.x); max.y = Mathf.Max(max.y, v.y); max.z = Mathf.Max(max.z, v.z);
//                if (0.0f < max.x && min.x < 1.0f && 0.0f < max.y && min.y < 1.0f && caMain.nearClipPlane < max.z && min.z < caMain.farClipPlane)
//                {
//                    isVisible = true;
//                }
//            }
