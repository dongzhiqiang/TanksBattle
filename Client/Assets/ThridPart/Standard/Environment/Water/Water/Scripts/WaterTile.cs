...
namespace UnityStandardAssets.Water
{
    ...
    public class WaterTile : MonoBehaviour
    {
        ...
        [ContextMenu("…Ë÷√renderTexture")]
        void SetRenderTextureToMaterial()
        {
            if (reflection == null)
                return;
            GameObject go = GameObject.Find("waterReflectionSceneCamera");
            if (go == null)
                return;
            Renderer r = this.GetComponent<Renderer>();
            if (r == null)
                return;
            r.sharedMaterial.SetTexture("_ReflectionTex", go.GetComponent<Camera>().targetTexture);
