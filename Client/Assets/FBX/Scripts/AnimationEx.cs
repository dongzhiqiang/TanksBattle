...
{
    public static List<AnimationClip> s_commonClips ;
    ...
    void Awake()
    {
        ...
        //加载下通用动作
        if(s_commonClips==null){
            s_commonClips =new List<AnimationClip>(Resources.Load<GameObject>("CommonClips").GetComponent<CommonClips>().m_commonClips);
        }
        
        AddClipIfNoExist(s_commonClips);
    }

    void Start()
    {
        if (currentAnimationData == null && animations.Count > 0)
        {
            Play(animations[0]);
        }
    ...
    public void DoFixedUpdate(float timeDelta)
    {
        //WrapMode emulator
        ...
        currentAnimationData.secondsPlayed += (timeDelta * animator.speed);
        ...
        if (currentAnimationData.secondsPlayed == currentAnimationData.length)
        {
            if (currentAnimationData.clip.wrapMode == WrapMode.Loop || currentAnimationData.clip.wrapMode == WrapMode.PingPong)
            {
                ...
                else if (currentAnimationData.clip.wrapMode == WrapMode.PingPong)
                {
            ...
            else if (currentAnimationData.timesPlayed == 0)
            {
                ...
                if (currentAnimationData.clip.wrapMode == WrapMode.ClampForever)
                    animator.speed = 0;
                else if (currentAnimationData.clip.wrapMode == WrapMode.Once)
                {
                    SetCurrentClipPosition(0);
                    animator.speed = 0;
                }
    ...
    {
        animations.Remove(GetAnimationData(name));
    }

    public void RemoveClip(AnimationClip clip)
    {
        
        animations.Remove(GetAnimationData(clip));
    ...
    public void AddClipIfNoExist(List<AnimationClip> clips)
    {
          HashSet<string> set = new HashSet<string>();
          foreach (AnimationData animData in animations)
          {
              set.Add(animData.clipName);
              set.Add(animData.clip.name);
          }

          foreach(AnimationClip c in clips){
              if (c!=null && !set.Contains(c.name))
              {
                  set.Add(c.name);
                  AddClip(c, c.name);
              }
    ...
    {
        if (GetAnimationData(newName) != null) Debuger.Log("An animation with the name '" + newName + "' already exists.");
    ...
    private void _playAnimation(AnimationData targetAnimationData, float blendingTime, float normalizedTime, bool mirror)
    {
        ...
        if (blendingTime == -1 && currentAnimationData!=null) blendingTime = currentAnimationData.transitionDuration;
        ...
        targetAnimationData.clip.wrapMode = targetAnimationData.wrapMode;
