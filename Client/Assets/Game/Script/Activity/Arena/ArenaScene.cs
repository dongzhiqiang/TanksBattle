...
public class ArenaScene : LevelBase
{
        ...
        foreach (Role pet in myPets)
        {
            if (pet != null)
            {
                ...
                if (petGuid == myPetFormation.GetPetGuid(enPetPos.pet1Main))
                {
                    ...
                    //如果对方有对应的神侍就设置仇恨目标
                    string enemyPetGuid = enemyPetFormation.GetPetGuid(enPetPos.pet1Main);
                    if (!string.IsNullOrEmpty(enemyPetGuid))
                    {
                        var tempPet = m_enemyHero.PetsPart.GetPet(enemyPetGuid);
                        if (tempPet != null)
                ...
                else if (petGuid == myPetFormation.GetPetGuid(enPetPos.pet2Main))
                {
                    ...
                    //如果对方有对应的神侍就设置仇恨目标
                    string enemyPetGuid = enemyPetFormation.GetPetGuid(enPetPos.pet2Main);
                    if (!string.IsNullOrEmpty(enemyPetGuid))
                    {
                        var tempPet = m_enemyHero.PetsPart.GetPet(enemyPetGuid);
                        if (tempPet != null)
    ...
    private void OnFriendRoleDead(object p, object p2, object p3, EventObserver ob)
    {
        ...
        {            
            ...
            var petFormation = hero.PetFormationsPart.GetCurPetFormation();
            Role myPet1 = petsPart.GetPet(petFormation.GetPetGuid(enPetPos.pet1Main));
            Role myPet2 = petsPart.GetPet(petFormation.GetPetGuid(enPetPos.pet2Main));
            //主角死了？把摄像机移到宠物那
