...
public class UIEquipPageUpgrade : MonoBehaviour
{
    ...
    //显示
    public void OnOpenPage(Equip equip)
    {
        ...
        { 
            ...
            {
                sourceCostItems = EquipAdvanceCostCfg.GetCost((int)(equipCfg.posIndex) + "_" + (equip.AdvLv), false);
            }
    ...
    void Awake()
    {
        ...
        actList1.Add(new GrowActFxPlay(GetComponentInParent<UIEquip>().m_updateArrowFx, true));
        ...
        actList3.Add(new GrowActFxPlay(GetComponentInParent<UIEquip>().m_equipFx));
        List<GrowAct> actList4 = new List<GrowAct>();
        actList4.Add(new GrowActFunc(() => UIMgr.instance.Get<UIEquip>().Refresh()));
        ...
        //m_upgradeFxMgr.m_callback = () => UIMgr.instance.Get<UIEquip>().Refresh();
        ...
        advActList2.Add(new GrowActFxPlay(GetComponentInParent<UIEquip>().m_equipFx));

        List<GrowAct> advActList3 = new List<GrowAct>();
        advActList3.Add(new GrowActFunc(() => UIMgr.instance.Get<UIEquip>().Refresh()));
        ...
        m_advanceFxMgr.m_flyIconFx = GetComponentInParent<UIEquip>().m_flyIconFx;
        //m_advanceFxMgr.m_callback = () => UIMgr.instance.Get<UIEquip>().Refresh();
    }
    ...
    void OnUpgrade()
    {
        ...
        if(m_equip.Level >= RoleMgr.instance.Hero.PropPart.GetInt(enProp.level))
        {
            UIMessage.Show("角色等级不足");
            return;
        }
        // check items
        ...
        NetMgr.instance.EquipHandler.SendUpgrade(RoleMgr.instance.Hero.GetString(enProp.guid), equipCfg.posIndex);
    ...
    void OnUpgradeOnce()
    {
        ...
        {
            // check items
            costItems = EquipAdvanceCostCfg.GetCost((int)(equipCfg.posIndex) + "_" + (m_equip.AdvLv), false);
        }
        else
        {
            if (m_equip.Level >= RoleMgr.instance.Hero.PropPart.GetInt(enProp.level))
        ...
        UIPowerUp.SaveOldProp(RoleMgr.instance.Hero);
        NetMgr.instance.EquipHandler.SendUpgradeOnce(RoleMgr.instance.Hero.GetString(enProp.guid), equipCfg.posIndex);
        m_updateButtonLock = true;
    }
    ...
    void OnAdvance()
    {
        ...
        EquipCfg equipCfg = EquipCfg.m_cfgs[m_equip.EquipId];
        // check items
        List<CostItem> costItems = EquipAdvanceCostCfg.GetCost((int)(equipCfg.posIndex) + "_" + (m_equip.AdvLv), false);
        int needItemId;
        if (!RoleMgr.instance.Hero.ItemsPart.CanCost(costItems, out needItemId))
        {
            ItemCfg itemCfg = ItemCfg.m_cfgs[needItemId];
        ...
        UIPowerUp.SaveOldProp(RoleMgr.instance.Hero);
        NetMgr.instance.EquipHandler.SendAdvance(RoleMgr.instance.Hero.GetString(enProp.guid), equipCfg.posIndex);
        m_updateButtonLock = true;
    }
    ...
    void OnUpgradeAll()
    {
        ...
        UIPowerUp.SaveOldProp(RoleMgr.instance.Hero);
        NetMgr.instance.EquipHandler.SendUpgradeAll(RoleMgr.instance.Hero.GetString(enProp.guid));
        m_updateButtonLock = true;
    }
